...
if (isset($_SESSION['curDomain'])) {
     ...
     */
    function demoGetAttributeValues($itemName, $attributeName) {
	global $mySDB;
...
<div style='font-family:sans-serif; background-color:#ddddff; font-size:small' >
    ...
    <h2>paws Simple DB Library Demo v0.01</h2>
<?
...
if (isset($_POST["listCurDomain"])) {			//******* List Current Domain
...
if (isset($_POST["putAttributes"]) || isset($_POST["replaceAttributes"])) {	//******* Put/replace Attributes
	...
	if (demoPutAttributes($_POST["itemName"], $row, $replace)) {
...
if (isset($_POST["putDefaultAttributes"]) ) {	//******* Put default Attributes
	...
	if (demoPutAttributes($_POST["itemName"], $row)) {
	    echo("<p>Put succeeded</p>");
	}
...
if (isset($_POST["getAttributes"]) || isset($_POST['getAttributeValues'])) {			//******* Get Attributes
	...
	if (isset($_POST["getAttributes"])) {
	    ...
	    demoGetAttributeValues($_POST["itemGName"], $_POST['getName']);
	}
    }
}
...
if (isset($_POST["deleteAttributes"])) {
	...
	if (strlen($_POST['n0']) > 0) {
	    if (strlen($_POST['v0']) > 0) {
		$attr[$_POST['n0']] = $_POST['v0'];
		demoDeleteAttributes($itemName,$attr);
	    } else {
		$attr = array($_POST['n0']=>NULL);
		demoDeleteAttributes($itemName,$attr);
	    }
	} else {
...
?>
	...
	<table>
		...
		<td>Set domain: <input name="domainName" type="text" maxlength=20 />
		    <input name="setDomain" type="submit" value="Set Domain" /></td></tr>
	</table>
	<p><strong>Create and Delete domains can take up to 10 seconds - be patient!</strong></p>
	<table>
	<tr><td><label for="createDomain">Domain to create: </label><input name="createDomain" type="text" maxlength=20 /></td>
	    <td><input type="submit" name="createTheDomain" value="Create Domain" /></p></td>
	    <td>&nbsp;</td><td><label for="deleteDomain">Domain to delete: </label><input name="deleteDomain" type="text" maxlength=20 /></td>
	...
	<table>
	    <tr><td><label for="itemName">Item Name (row name): </label><input name="itemName" type="text" maxlength=20 /></td></tr>
	    <tr><td><small>Allows up to 4 name/value attributes</small></td><td><small>(Only one value per attribute so far)</small></td></tr>
	    <tr><td>0:<input name="n0" type="text" maxlength=20 /> / <input name="v0" type="text" maxlength=20 /></td>
	      <td>1:<input name="n1" type="text" maxlength=20 /> / <input name="v1" type="text" maxlength=20 /></td></tr>
	    <tr><td>2:<input name="n2" type="text" maxlength=20 /> / <input name="v2" type="text" maxlength=20 /></td>
	       <td>3:<input name="n3" type="text" maxlength=20 /> / <input name="v3" type="text" maxlength=20 /></td></tr>
	...
	<table>
	    <tr><td><label for="itemGName">Item Name (row name): </label><input name="itemGName" type="text" maxlength=20 /></td></tr>
	    <tr><td><small>Gets all attributes for item </small><input type="submit" name="getAttributes" value="Get Attributes" /></td></tr>
	    <tr><td>Attribute name:<input name="getName" type="text" maxlength=20 />
		<input type="submit" name="getAttributeValues" value="Get Attribute Value(s)" /></td></tr>
	</table>
	</fieldset>
	...
	<table>
	    ...
	    <tr><td>Attribute:<input name="n0" type="text" maxlength=20 /> / <input name="v0" type="text" maxlength=20 /></td>
	...
	<table>
	<tr><td>List current Domain: <input name="listCurDomain" type="submit" value="List Cur Domain" /></td></tr>
